...
(function () {
  ...
  const KEYS = {
    ...
    exportSchemaOpen: "lfg_export_schema_open_v1",
    ...
    qrDetailsOpen: "lfg_qr_details_open_v1",
  ...
  function loadQrDetailsOpen() {
    try { return localStorage.getItem(KEYS.qrDetailsOpen) === "1"; }
    catch { return false; }
  }

  function saveQrDetailsOpen(isOpen) {
    try { localStorage.setItem(KEYS.qrDetailsOpen, isOpen ? "1" : "0"); }
    catch {}
  }

  function clearQrDetailsOpen() {
    try { localStorage.removeItem(KEYS.qrDetailsOpen); }
    catch {}
  }
  ...
  function getExportFields() {
    const v = lsGetJson(KEYS.exportFields);
    return Array.isArray(v) ? v : null;
  }

  function setExportFields(fields) {
    if (!Array.isArray(fields)) return;
    lsSetJson(KEYS.exportFields, fields);
  }

  function clearExportFields() {
    lsRemove(KEYS.exportFields);
  }

  function loadExportSchemaOpen() {
    return lsGet(KEYS.exportSchemaOpen) === "1";
  }

  function saveExportSchemaOpen(isOpen) {
    lsSet(KEYS.exportSchemaOpen, isOpen ? "1" : "0");
  }

  function clearExportSchemaOpen() {
    lsRemove(KEYS.exportSchemaOpen);
  }
  ...
  function clearSingleInputs() {
    setValueAndDispatch(ctx.urlInputEl, "");
    setValueAndDispatch(ctx.fileNameInputEl, "");
    lsSet(KEYS.singleUrl, "");
    lsSet(KEYS.singleFileName, "");
  }

  function clearBatchInputs() {
    setValueAndDispatch(ctx.batchInputEl, "");
    setValueAndDispatch(ctx.zipNameInputEl, "");
    lsSet(KEYS.batchText, "");
    lsSet(KEYS.zipName, "");
  }
  ...
  window.App.state = {
    ...
    setExportFields,
    clearExportFields,
    loadExportSchemaOpen,
    saveExportSchemaOpen,
    clearExportSchemaOpen,
