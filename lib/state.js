...
(function () {
  ...
  function clearSingleInputs() {
    // 1) clear DOM (so UI matches immediately)
    if (ctx.urlInputEl) ctx.urlInputEl.value = "";
    if (ctx.fileNameInputEl) ctx.fileNameInputEl.value = "";

    // 2) clear persisted values
    lsRemove(KEYS.singleUrl);
    lsRemove(KEYS.singleFileName);

    // 3) (optional but helps) persist the cleared state explicitly
    // This prevents any other code from restoring old values later.
  ...
  function clearBatchInputs() {
    // 1) clear DOM
    if (ctx.batchInputEl) ctx.batchInputEl.value = "";
    if (ctx.zipNameInputEl) ctx.zipNameInputEl.value = "";

    // 2) clear persisted values
    lsRemove(KEYS.batchText);
    lsRemove(KEYS.zipName);

    // 3) (optional but helps) persist cleared state explicitly
