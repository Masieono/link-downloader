...
(function () {
  ...
  function init(deps) {
    ...
    // Optional: single inputs + buttons
    const fileNameInputEl = d.fileNameInputEl || null;

    const downloadBtnEl = d.downloadBtnEl || null;
    const copyBtnEl = d.copyBtnEl || null;
    const openNowBtnEl = d.openNowBtnEl || null;
    const resetBtnEl = d.resetBtnEl || null;

    // Optional: batch buttons
    ...
    // Optional: history UI (used by factory reset)
    const historyDetailsEl = d.historyDetailsEl || null;

    // Optional: batch option toggles (used by reset/factory reset flows)
    ...
    // Optional: QR <details> / summary / summary text (avoid querying DOM inside actions)
    const qrDetailsEl = d.qrDetailsEl || null;
    const qrSummaryEl = d.qrSummaryEl || null;
    const qrSummaryTextEl = d.qrSummaryTextEl || null;
    ...
    function flashButton(btn, text = "Copied!", ms = 1000) {
      ...
      // For <button> (or anything else), prefer a label span if present, otherwise a text node
      function getLabelNode(el) {
        // If you already have a dedicated label span, we’ll use it automatically:
        // <button><svg ...></svg><span class="btn-label">Copy</span></button>
        const explicit = el.querySelector?.(".btn-label,[data-btn-label]");
        if (explicit) return explicit;

        // Otherwise, find the first *direct* text node with non-whitespace content
        const nodes = Array.from(el.childNodes || []);
        const textNode = nodes.find(
          (n) => n.nodeType === Node.TEXT_NODE && String(n.nodeValue || "").trim().length
        );
        if (textNode) return textNode;

        // If no text node exists (e.g., icon-only button), create one at the end
      ...
      const st = btn[stateKey] || {
        timer: null,
        prevText: null,
        prevValue: null,
        target: null, // text node or element used for label updates
      };
      btn[stateKey] = st;
      ...
      if (isInputLike) {
        ...
        // Capture original label once per flash cycle
        if (!st.target) st.target = getLabelNode(btn);

        if (st.prevText == null) {
          st.prevText =
            st.target.nodeType === Node.TEXT_NODE ? st.target.nodeValue : st.target.textContent;
        }

        if (st.target.nodeType === Node.TEXT_NODE) st.target.nodeValue = text;
        else st.target.textContent = text;
      }
      ...
      st.timer = window.setTimeout(() => {
        if (isInputLike) {
          ...
          if (st.target.nodeType === Node.TEXT_NODE) st.target.nodeValue = st.prevText ?? "";
          else st.target.textContent = st.prevText ?? "";
        }

        btn.classList?.remove?.("flash");

        // Reset for next time
        btn[stateKey] = { timer: null, prevText: null, prevValue: null, target: null };
      }, ms);
    }
    ...
    function refreshSingleActionsEnabled() {
      const raw = urlInputEl?.value?.trim() || "";
      const norm = raw ? App.url.normalizeUrl(raw) : { ok: false };

      if (downloadBtnEl) downloadBtnEl.disabled = !norm.ok;
      if (copyBtnEl) copyBtnEl.disabled = !norm.ok;
      if (openNowBtnEl) openNowBtnEl.disabled = !norm.ok;
      if (resetBtnEl) resetBtnEl.disabled = !raw;
    }
    ...
    function setBatchDownloadEnabled({ hasValid, hasDeduped }) {
      if (downloadZipBtnEl) downloadZipBtnEl.disabled = !hasDeduped;
      if (downloadCsvBtnEl) downloadCsvBtnEl.disabled = !hasDeduped;
      if (downloadJsonBtnEl) downloadJsonBtnEl.disabled = !hasDeduped;
      if (downloadBatchFileBtnEl) downloadBatchFileBtnEl.disabled = !hasValid;
    }
    ...
    // QR details (collapsible) open persistence + validity gating

    function isQrPanelOpen() {
      return !!(qrDetailsEl && qrDetailsEl.open && qrPanelEl && qrPanelEl.hidden === false);
    }

    function isSingleUrlValid() {
      const raw = urlInputEl?.value?.trim() || "";
      const norm = App.url.normalizeUrl(raw);
      return !!norm.ok;
    }

    function setQrUiEnabled(enabled) {
      const els = [
        qrFgEl, qrBgEl, qrTransparentEl, qrMarginEl, qrSizeEl, qrEccEl,
        qrCopyUrlBtnEl, qrCopySvgBtnEl, qrDlPngBtnEl, qrDlSvgBtnEl,
      ];
      els.forEach((el) => { if (el) el.disabled = !enabled; });
    }

    function updateQrSummaryText() {
      if (!qrSummaryTextEl) return;

      if (!isSingleUrlValid()) {
        qrSummaryTextEl.textContent = "Enter a valid URL";
        return;
      }
      qrSummaryTextEl.textContent = "Customize + download";
    }

    function wireQrDetails() {
      if (!qrDetailsEl || !qrSummaryEl) return;

      const detailsEl = qrDetailsEl;
      const summaryEl = qrSummaryEl;

      // restore open state
      const shouldOpen = App.state?.loadQrDetailsOpen?.() ?? false;
      detailsEl.open = shouldOpen;

      // apply initial validity state
      const valid = isSingleUrlValid();
      if (qrPanelEl) qrPanelEl.hidden = !(detailsEl.open && valid);
      setQrUiEnabled(valid);
      updateQrSummaryText();

      // prevent opening when invalid
      summaryEl.addEventListener("click", (e) => {
        if (!isSingleUrlValid()) {
          e.preventDefault();
          detailsEl.open = false;
          if (qrPanelEl) qrPanelEl.hidden = true;
          setQrUiEnabled(false);
          updateQrSummaryText();
          ui.showToast("Enter a valid URL to use QR.", "error", 2200);
        }
      });
      ...
      detailsEl.addEventListener("toggle", () => {
        ...
        // if user tries to open while invalid, force closed
        if (nowOpen && !isSingleUrlValid()) {
          detailsEl.open = false;
          App.state?.clearQrDetailsOpen?.();
          if (qrPanelEl) qrPanelEl.hidden = true;
          setQrUiEnabled(false);
          updateQrSummaryText();
          return;
        ...
        App.state?.saveQrDetailsOpen?.(nowOpen);

        // Show/hide inner panel + render when opening
        if (qrPanelEl) qrPanelEl.hidden = !(nowOpen && isSingleUrlValid());
        setQrUiEnabled(isSingleUrlValid());
        updateQrSummaryText();

        if (nowOpen && isSingleUrlValid()) {
          scheduleQrRefresh("details-open");
        }
      });
    }
    ...
    function clearTextArea() {
      App.state?.clearSingleInputs?.();

      clearQrPreviewUi();
      updateQrSummaryText();

      refreshSingleActionsEnabled();
      if (typeof updatePreview === "function") updatePreview();

      if (qrDetailsEl) qrDetailsEl.open = false;
      App.state?.clearQrDetailsOpen?.();
      if (qrPanelEl) qrPanelEl.hidden = true;

      ui?.showToast?.("Cleared.", "success", 1400);
    }

    function clearBatchTextArea() {
      App.state?.clearBatchInputs?.();

      // Hard-disable immediately (no reliance on buildBatchPlanFromUI stability)
      setBatchDownloadEnabled({ hasValid: false, hasDeduped: false });
      setCopyBatchEnabled(false);
      setOpenFirstEnabled(false);
      setBatchClearEnabled(false);

      // Then let the normal refresh logic re-derive states safely
      try {
        refreshBatchCopyEnabled();
      } catch {
        // If something is unexpectedly wrong, keep the forced-disabled state.
      }
    ...
    function handleFactoryReset() {
      try {
        ...
        if (historyDetailsEl) historyDetailsEl.open = false;
        ...
        // Close QR UI + cleanup any object URL
        if (qrDetailsEl) qrDetailsEl.open = false;

        if (qrPanelEl) qrPanelEl.hidden = true;

        // Clear any rendered QR + revoke object URL
        clearQrPreviewUi();

        // Clear persisted open states
        App.state?.clearQrDetailsOpen?.();


        // --- Reset QR UI back to defaults (predictable factory reset)
        try {
          if (qrFgEl) qrFgEl.value = "#000000";
          if (qrBgEl) qrBgEl.value = "#ffffff";
          if (qrTransparentEl) qrTransparentEl.checked = false;

          if (qrMarginEl) qrMarginEl.value = 4;
          if (qrSizeEl) qrSizeEl.value = "medium";
          if (qrEccEl) qrEccEl.value = "M";

          // If contrast warning is visible, hide it (will re-evaluate on next render)
          if (qrWarnEl) qrWarnEl.style.display = "none";
        } catch {}
        ...
        if (typeof updatePreview === "function") updatePreview();
        if (typeof refreshSingleActionsEnabled === "function") refreshSingleActionsEnabled();
