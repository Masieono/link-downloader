  ...
  function init(deps) {
    ...
    function refreshBatchCopyEnabled() {
      try {
        ...
        setBatchDownloadEnabled(hasDeduped);
    ...
    function setSingleActionsEnabled(enabled) {
      if (downloadBtnEl) downloadBtnEl.disabled = !enabled;
      if (copyBtnEl) copyBtnEl.disabled = !enabled;
      if (openNowBtnEl) openNowBtnEl.disabled = !enabled;
      if (resetBtnEl) resetBtnEl.disabled = !enabled;
    }
    ...
    function isQrPanelOpen() {
      ...
      return !!(detailsEl && detailsEl.open && qrPanelEl && qrPanelEl.hidden === false);
    }

    // QR details (collapsible) open persistence + validity gating

    function saveQrDetailsOpen(isOpen) {
      try { localStorage.setItem(QR_DETAILS_OPEN_KEY, isOpen ? "1" : "0"); } catch {}
    }

    function loadQrDetailsOpen() {
      try { return localStorage.getItem(QR_DETAILS_OPEN_KEY) === "1"; } catch { return false; }
    }
    ...
    function updateQrSummaryText() {
      const el = document.getElementById("qrSummaryText");
      if (!el) return;
      ...
      el.textContent = "Customize + download";
    }

    function wireQrDetails() {
    const detailsEl = document.getElementById("qrDetails");
    const summaryEl = document.getElementById("qrSummary");
    if (!detailsEl || !summaryEl) return;

    // restore open state
    const shouldOpen = loadQrDetailsOpen();
    ...
    function handleFactoryReset() {
      try {
        ...
        // Close the QR collapsible too (factory reset should fully close it)
        try {
          const qrDetailsEl = document.getElementById("qrDetails");
          if (qrDetailsEl) qrDetailsEl.open = false;
          // Also clear the persisted open state for QR collapsible (if you store one)
          localStorage.removeItem("lfg_qr_details_open_v1");
        } catch {}

        // Clear QR "open" persistence so factory reset is truly clean
        try { localStorage.removeItem("lfg_qr_panel_open_v1"); } catch {}
