...
(function () {
  ...
  function render() {
    ...
    if (items.length === 0) {
      el.innerHTML = `<div class="hint history-empty">No history yet. Actions like Download/Open/Copy/Import/Remove/Restore will appear here.</div>`;
      return;
    }
    ...
    for (const item of items) {
      ...
      if (isSingle) {
        actionsHtml =
          btnHtml({ act: "copy", label: "Copy URL", icon: "copy", classes: "btn secondary small" }) +
          btnHtml({ act: "open", label: "Open", icon: "open", classes: "btn secondary small" }) +
          btnHtml({ act: "download", label: "Download", icon: "download", classes: "btn secondary small" }) +
          btnHtml({ act: "load", label: "Restore", icon: "restore", classes: "btn secondary small" }) +
          btnHtml({ act: "remove", label: "Remove", icon: "remove", classes: "btn secondary small danger" });
        ...
        actionsHtml =
          btnHtml({ act: "restore", label: "Restore", icon: "restore", classes: "btn secondary small" }) +
          btnHtml({ act: dlAct, label: dlLabel, icon: "download", classes: "btn secondar small" }) +
          btnHtml({ act: "copyUrls", label: "Copy URL list", icon: "copy", classes: "btn secondary small" }) +
          btnHtml({ act: "remove", label: "Remove", icon: "remove", classes: "btn secondary small danger" });

      } else if (isCopyEvent) {
        actionsHtml =
          btnHtml({ act: "copyText", label: "Copy", icon: "copy", classes: "btn secondary small" }) +
          btnHtml({ act: "remove", label: "Remove", icon: "remove", classes: "btn secondary small danger" });

      } else {
        actionsHtml = btnHtml({ act: "remove", label: "Remove", icon: "remove", classes: "btn secondary small danger" });
      }
  ...
  function iconSvg(name) {
    const map = {
      ...
      download: "i-download",
      remove: "i-clear",
      restore: "i-restore", // reuse open icon unless you later add a dedicated restore icon
    };
  ...
  function btnHtml({ act, label, icon, classes = "secondary small" }) {
    const iconPart = icon
      ? `<span class="btn-icon">${iconSvg(icon)}</span>`
      : "";

    return `
      <button type="button" class="${classes}" data-act="${escapeHtml(act)}">
        ${iconPart}
  ...
  function flashElText(el, text = "Copied!", ms = 1000) {
    ...
    const st = el[stateKey] || { timer: null, prevHtml: null };
    el[stateKey] = st;
    ...
    st.timer = setTimeout(() => {
      el.innerHTML = st.prevHtml;
      el.classList.remove("flash");
      el[stateKey] = { timer: null, prevHtml: null };
    }, ms);
  }
