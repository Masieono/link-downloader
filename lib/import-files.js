...
(function () {
  ...
  function extractUrlsFromHtmlFile(htmlText) {
    ...
    // 1) meta refresh redirect (robust: quoted/unquoted content, varied spacing)
    let m = text.match(/http-equiv\s*=\s*["']?refresh["']?[^>]*content\s*=\s*["']?[^"'>]*url\s*=\s*([^"'>\s;]+)[^"'>]*["']?/i);
  ...
  function routeImportedUrls(urls, sourceLabel) {
    ...
    if (unique.length === 1 && currentMode !== "batch") {
      ...
      setValueAndDispatch(ctx.urlInputEl, unique[0]);
    ...
    if (typeof ctx.appendLinesToBatch === "function") {
      ...
      // appendLinesToBatch sets .value but doesn't dispatch; we need listeners to run
      setValueAndDispatch(ctx.batchInputEl, ctx.batchInputEl?.value || "");
    } else if (ctx.batchInputEl) {
      setValueAndDispatch(ctx.batchInputEl, unique.join("\n"));
    }
  ...
  async function importAnyFile(file) {
    ...
    // Content sniff: Windows .url shortcut (handles odd filenames like *.url.download)
    if (/^\s*\[InternetShortcut\]\s*$/im.test(text) && /^\s*URL\s*=/im.test(text)) {
      const u = extractUrlFromWindowsUrlFile(text);
      if (u) return routeImportedUrls([u], ".url shortcut");
      return App.ui.showToast("That shortcut did not contain a readable URL= entry.", "error", 3200);
    }
    ...
    // .webloc (plist)
    const isPlist = /<plist[\s>]/i.test(text);
    const looksLikeWebloc = /<key>\s*URL\s*<\/key>/i.test(text);

    if (name.endsWith(".webloc") || (isPlist && looksLikeWebloc)) {
      ...
      // If it's some other plist/XML, don't claim it's a webloc—just continue.
    }
        ...
        if (parsed.kind === "batch") {
          ...
          if (!replace) {
            ...
            if (typeof ctx.appendLinesToBatch === "function") {
              ...
              setValueAndDispatch(ctx.batchInputEl, next);
            }
