...
(function () {
  ...
  function wireExplainers() {
    // Update explainers whenever the radios change.
    // (This preserves existing state persistence behavior; we just reflect it.)
    document.addEventListener("change", (e) => {
      const t = e.target;
      if (!(t instanceof Element)) return;

      if (t.matches('input[name="fileType"]')) {
        updateFileTypeHelp();
        return;
      }
      if (t.matches('input[name="urlMode"]')) {
        updateUrlModeHelp();
        return;
      }
